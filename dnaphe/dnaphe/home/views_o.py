...
from .forms import CommentForm, LinkPostForm
...
def index(request):
    ...
    context = [i for i in Post.objects.order_by('-date_posted', '-upvote')]
...
def popular(request):
    ...
    r = [i for i in q if i.was_published_recently()]
...
def botPost(request):
    ...
    for i in range(len(title)):
        if title[i] not in [i.title for i in Post.objects.all()]:
            try:
                q = Post(
                    topic = Topic.objects.get(topic_text='OnlineKhabar-News'),
...
def kantipurbotPost(request):
    ...
    for i in range(len(title)):
        if title[i] not in [i.title for i in Post.objects.all()]:
            try:
                q = Post(
                    topic = Topic.objects.get(topic_text='KantipurDaily-News'),
                    language = Lang_tag.objects.get(lang_tag_text='Nepali'),
                    title = title[i],
                    content = content[i],
                    date_posted = timezone.now(),
